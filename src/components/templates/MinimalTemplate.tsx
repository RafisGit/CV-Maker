...
export default function MinimalTemplate({ data, colorTheme }: { data: CVData; colorTheme: ColorTheme }) {
  const { personalInfo, education, experience, skills, projects, certifications } = data;

  return (
    <div className="bg-white text-gray-900 w-full px-8 py-6" style={{ fontFamily: "'Georgia', 'Times New Roman', serif" }}>
      {/* Header */}
      <div className="text-center border-b pb-4 mb-5" style={{ borderColor: colorTheme.sectionBorder }}>
      ...
      <div className="space-y-4">
        {personalInfo.summary && (
          <section>
            <h2 className="text-xs font-normal uppercase tracking-[0.2em] mb-1.5" style={{ color: colorTheme.secondary }}>
              Summary
            </h2>
        ...
        {experience.length > 0 && (
          <section>
            <h2 className="text-xs font-normal uppercase tracking-[0.2em] mb-2" style={{ color: colorTheme.secondary }}>
              Experience
            </h2>
            <div className="space-y-3">
              {experience.map((exp) => (
                <div key={exp.id}>
                  ...
                  {exp.description && <p className="text-xs text-gray-700 mt-1 whitespace-pre-line">{exp.description}</p>}
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="space-y-2">
              {education.map((edu) => (
                <div key={edu.id}>
                  ...
                  {edu.description && <p className="text-xs text-gray-700 mt-0.5">{edu.description}</p>}
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="space-y-2">
              {projects.map((project) => (
                <div key={project.id}>
                  <h3 className="text-sm font-semibold">
                    {project.name}
                    {project.link && <span className="text-xs font-normal text-gray-500 ml-2">{project.link}</span>}
                  </h3>
                  {project.technologies && <p className="text-xs text-gray-500 italic">{project.technologies}</p>}
                  {project.description && <p className="text-xs text-gray-700 mt-0.5">{project.description}</p>}
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="space-y-1">
              {certifications.map((cert) => (
                <div key={cert.id} className="text-xs">
                  ...
                  {cert.issuer && <span className="text-gray-600"> &ndash; {cert.issuer}</span>}
                  {cert.date && <span className="text-gray-500 ml-1">({formatDate(cert.date)})</span>}
                </div>
              ))}
            </div>
          </section>
